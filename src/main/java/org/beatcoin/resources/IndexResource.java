...
public class IndexResource {
	...
	@Inject public IndexResource(ServletRequest request,
			ServletContext servletContext, AddressPool addressPool) {
	...
	public Response index(@HeaderParam("Accept-Language") String lng){
		...
		data.put("resPath", BitcoinIServletConfig.resPath);
		data.put("basePath", BitcoinIServletConfig.basePath);
		Iterator<String> i = addressPool.getPools().keySet().iterator();
		if (i.hasNext()){
			data.put("defaultAccount", i.next());
		}
